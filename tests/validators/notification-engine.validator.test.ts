...
describe('notification-engine validator', () => {
  it('varsayilan config semaya uyar ve her kural icin cooldown kuraldan gelir', () => {
    ...
    for (const key of LIFECYCLE_RULE_KEYS) {
      expect(DEFAULT_ENGINE_CONFIG.rules[key].cooldown_days).toBe(LIFECYCLE_RULES_BY_KEY[key].defaultCooldownDays);
    }
  });
  ...
  it('bos/eksik JSON varsayilanlarla birlesir, bilinmeyen anahtar atilir', () => {
    ...
    expect(config.rules.lifecycle_winback).toEqual({ enabled: false, cooldown_days: 30 });
  ...
  it('form: isaretsiz checkbox false, sayilar parse edilir, bos alan mevcut degeri korur', () => {
    const body = {
      ...
      rule_lifecycle_winback_cooldown: '14',
    };
    ...
    expect(parsed.data.rules.lifecycle_winback).toEqual({ enabled: true, cooldown_days: 14 });
  ...
  it('form: aralik disi deger reddedilir', () => {
    ...
    expect(parseEngineConfigForm({ rule_lifecycle_winback_cooldown: '0' }, DEFAULT_ENGINE_CONFIG).success).toBe(false);
