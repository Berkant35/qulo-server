...
describe('sendMinuteFor / variantFor / isRecurrenceDay (saf)', () => {
  ...
  it('gonderimde kullanicinin dili secilir: ar kullanicisina ar satiri, fr kullanicisina etiketsiz yedek', async () => {
    ...
    // Ikisi de lng 46 → UTC+3; pencere 0-24 ve tik 20:59Z = yerel 23:59 → gunun her slotu gecmis
    const seed = tables({
      campaigns: [campaign({ variants, window_start_hour: 0, window_end_hour: 24 })],
      users: [user({ id: 'A', locale: 'ar', lng: 46 }), user({ id: 'F', locale: 'fr', lng: 46 })],
      notification_engine_config: [{ id: 1, config: { holdout_pct: 0 } }],
    });
    const { send, campaignRecurringService } = await boot(seed);
    const r = await campaignRecurringService.dispatch(new Date('2026-09-25T20:59:00.000Z'));
